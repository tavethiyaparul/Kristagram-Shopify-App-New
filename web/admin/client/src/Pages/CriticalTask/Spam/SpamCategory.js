...
const SpamCategory = () => {
  ...
  const createSpamAPI = async (content, status) => {
    const data = {
      content: content,
      status: status,
    };
    const res = await axios.post(`/api/v1/newspam`, data);
    console.log("&&&", res);
    return res;
  };
  const getSpamAPI = async () => {
    setLoader(true);
    const res = await axios.get(`/api/v1/getspam`);
    console.log("&&&", res.data.spam);
  ...
  const columns = [
    {
      ...
      renderCell: (params) => {
        return (
          <Stack>
            <span>{params.row.content ? capitalizeFirst(params.row.content) : "-"}</span>
          </Stack>
        );
      },
    },
          ...
          <Stack>
            <Switch
              ...
              onChange={async (e) => {
                ...
                const response = await updateSpamAPI(data);
              }}
            />
    ...
    <div style={{ padding: "10px" }}>
      {loader ? (
        ...
        <div>
          ...
          >
            <DialogTitle id="alert-dialog-title">
              {"Are you sure you went to delete spam category ?"}
            </DialogTitle>
